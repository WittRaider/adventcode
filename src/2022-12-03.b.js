...
function getOverlap3(first,second,third) {
    let overlap = [];
    let charOverlaps = '';
    for(let i=0;i<first.length;i++) {
        let char1 = first[i];
...
groups.forEach(group => {
    group.overlap = getOverlap3(group[0].all,group[1].all,group[2].all);
    prio += group.overlap.prio;
});
