...
let current = null;
function findDirs(dir,sizeToFree) {
    if(dir.getSize() >= sizeToFree) {
        if(current === null) current = dir;
        else {
            if(dir.getSize() < current.getSize()) {
                current = dir;
            }
        }       
    }
    dir.getDirs().forEach(dir => {
        findDirs(dir,sizeToFree);
    });
}

let currentSize = 70000000 - root.getSize();
let sizeToFree = 30000000 - currentSize;

findDirs(root,sizeToFree);
debug(current.getSize());
